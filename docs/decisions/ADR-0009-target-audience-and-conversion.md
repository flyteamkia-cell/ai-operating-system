...
**Conversion event (Track B): entry into the Telegram channel.** Other funnel stages (VIP, copy trading, broker signup) exist and matter, but a single measurable conversion event is used while the loop is being validated. Multi-stage funnel CTAs are deferred until the Telegram step is measurably working.
